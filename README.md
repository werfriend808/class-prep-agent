...
### 18-6-4. 문항 내용 품질 문제 추가 대응 (2026-08-26)

18-6-3을 고친 뒤 Stanley가 여러 주제(덧셈, 곱셈, 동물의 한살이)로 실제 Google Forms를 만들어보고 문항을 직접 검산해봤는데, "정답 인덱스가 해설과 어긋나는" 것과는 다른 층위의 문제가 더 나왔습니다 — 과목을 가리지 않고(수학뿐 아니라 과학에서도) 나타났습니다.

- **문항 문장이 완결되지 않음**: "다음 중 어류인 생선은 다음과 다르게 생겼습니다", "246 + 173 = ___" 문항에 "다음 보기중 결과값이 아닌것은 다음과 같습니다"가 덧붙는 등, 질문문이 아니라 서술형·메타적인 문장으로 끝나서 의미 파악이 안 되는 경우.
- **빈칸/식 자체가 빠짐**: "다음 덧셈식에서 빈칸에 들어갈 알맞은 숫자를 찾으세요"라고만 하고, 정작 그 덧셈식이 문항 텍스트에 없는 경우.
- **보기 동점**: "15+27"(42), "12+30"(42), "30+12"(42)처럼 순서만 바꾼 계산식 세 개가 실제로는 같은 값이라 "가장 큰/같은 것을 고르세요" 유형 문항의 정답이 여러 개가 되는 경우. 곱셈에서도 "3x4"와 "4x3"이 둘 다 12로 동일한 사례가 있었습니다.
- **정답 자체가 틀림**: 246+173은 419인데 `correct_answer`가 410으로 나온 경우 — 18-6-3에서 고친 "인덱스 세기 실수"와 달리, 이번엔 정답 텍스트 자체가 계산과 다른 경우입니다.
- **사실 관계상 정답이 둘 이상**: "완전 탈바꿈을 거치는 곤충은?" 보기에 장수풍뎅이와 모기가 같이 있는데, 둘 다 완전변태 곤충이라 정답이 하나로 안 좁혀지는 경우.

이 중 앞의 셋(문장 완결성, 식 누락, 동점)은 원인이 비교적 뚜렷해서 대응했지만, 뒤의 둘(계산 자체가 틀림, 사실관계상 정답이 여럿)은 LLM이 그 순간 "생각"을 잘못한 결과라 프롬프트만으로는 완전히 근절한다고 장담할 수 없습니다 — 임의의 과목(수학뿐 아니라 국어·과학·사회 등)에 다 통하는 일반적인 정답 검증 로직을 코드로 짜기 어렵기 때문입니다. 그래서 두 갈래로 대응했습니다.

1. **프롬프트 지시 구체화** (`_build_prompt()`): "question은 완결된 질문문/지시문으로 끝낼 것", "계산식이 필요하면 그 식 자체를 문항에 포함할 것", "부정형 문항은 긍정·부정을 섞지 말 것", "correct_answer를 정하기 전에 실제로 계산/사실을 확인해서 정답이 명확히 하나뿐인지 확인할 것"을 명시적으로 추가했습니다. 추가 LLM 호출 없이 프롬프트 문구만 강화한 것이라 비용은 그대로입니다. 다만 이건 "지시"일 뿐이라 완전한 보장은 아닙니다.
2. **기계적으로 검증 가능한 부분만 코드로 차단**: `_parse_quiz_json()`에 두 검증을 추가했습니다 — ① 보기 텍스트가 완전히 겹치면(중복 보기) 과목과 무관하게 즉시 `QuizError`. ② 보기 전부가 `_try_eval_simple_expr()`로 계산 가능한 "숫자 연산자 숫자" 형태의 단순 산술식일 때만(예: "15+27", "3x4") 값을 계산해서 동점이 있으면 `QuizError`. 이 조건을 엄격하게 좁힌 이유는, 결과값 자체를 고르는 문항(보기가 "419","410"처럼 숫자만 있는 경우)이나 수학이 아닌 과목에는 이 패턴이 아예 안 나와서 오탐 없이 정확히 겨냥한 사례만 걸러내기 위해서입니다. 두 검증 모두 실패하면 기존 1회 재시도 정책이 그대로 작동합니다.

"계산 자체가 틀림"과 "사실관계상 정답이 여럿"은 여전히 남아있는 한계로 20번(알려진 제한사항)에 적어뒀습니다 — 필요하면 나중에 별도의 검증 LLM 호출을 한 번 더 추가하는 방안도 있지만, 이 프로젝트의 "크레딧 없이 개발/검증" 방침(8-5 참고)과 맞지 않아 지금은 프롬프트 개선 선에서 멈췄습니다. **후속: 이 방안은 18-6-5에서 실제로 추가됐습니다.**

### 18-6-5. 검수 LLM 호출 추가 (2026-08-26)

18-6-4에서 "코드로 근절 못 함"으로 남겨뒀던 유형이 실제로 또 발견됐습니다 — 영어 문법 퀴즈에서 "다음 중 'to 부정사'가 형용사적으로 사용되지 않은 것을 고르세요" 문항의 보기로 "happy to see you", "difficult to understand", "easy to learn"을 냈는데, 이 셋은 전부 형용사적 용법이 아니라 부사적 용법입니다(형용사적 용법은 "a book to read"처럼 to부정사가 명사를 뒤에서 꾸미는 경우입니다). 즉 "보기 중 셋은 형용사적 용법이고 하나만 아니다"라는 문항의 전제 자체가 틀렸습니다. 체크된 정답("interested in playing"은 애초에 to부정사가 아니라서 트리비얼하게 정답이긴 합니다)이 결과적으로 틀리진 않았지만, 문항이 잘못된 문법 분류를 전제로 깔고 있어서 학생에게 오개념을 심어줄 수 있는 문제였습니다.

이건 18-6-4의 동점 검증(`_try_eval_simple_expr`)처럼 값을 계산해서 기계적으로 잡을 수 있는 유형이 아니라, 그 과목의 실제 지식(이 경우 영문법 분류 체계)이 있어야 판단할 수 있는 오류입니다 — 임의의 과목에 다 통하는 코드 검증을 만들 수 없습니다. Stanley가 "크레딧/시간이 더 들어도 검증 단계를 추가해달라"고 명시적으로 요청해서, 18-6-4에서 보류했던 방안(별도의 검증 LLM 호출)을 실제로 추가했습니다.

- `quiz._generate_once()`가 `_parse_quiz_json()`으로 문항을 파싱한 직후, `_verify_questions()`를 호출합니다. 이 함수는 문항을 생성한 것과 같은 LLM에게 방금 만든 문항(질문, 보기, [표시된 정답] 표시, 해설)을 그대로 다시 보여주고 "① 표시된 정답이 실제로 맞는가, ② 정답이 하나로 좁혀지는가, ③ 문항이 전제하는 개념/분류/문법이 정확한가, ④ 문항이 완결되고 명확한가"를 검수시킵니다(`_build_verification_prompt()`) — 문항을 다시 쓰게 하는 게 아니라 valid/invalid 판정만 받습니다.
- 검수 결과가 하나라도 invalid면 `QuizError`를 올립니다. `_generate_once()`는 형식 오류든 검수 실패든 구분 없이 똑같이 실패로 취급되므로, `generate_quiz()`에 이미 있던 1회 재시도 정책이 검수 실패에도 자연스럽게 그대로 적용됩니다 — 별도의 재시도 루프를 새로 만들 필요가 없었습니다.
- 검수 호출 자체가 실패하거나(네트워크/크레딧 오류) 검수 응답이 JSON으로 파싱이 안 되거나 문항 개수와 안 맞으면(`_parse_verification_json()`이 `None` 반환), 조용히 건너뛰고 원래 생성 결과를 그대로 씁니다 — 검수는 "있으면 좋은 안전장치"지 필수 관문이 아니라고 판단했습니다. 검수 인프라 문제 때문에 정상적으로 만들어진 퀴즈까지 버리는 건 배보다 배꼽이 큰 상황이라고 봤습니다.

**트레이드오프**: 최악의 경우 LLM 호출이 기존 "생성 최대 2회"에서 "생성 최대 2회 + 검수 최대 2회" = 최대 4회로 늘어납니다. "크레딧 없이 개발" 방침(8-5)과 정확히 반대 방향의 결정이라, 이번엔 Stanley가 정확도를 우선하기로 명시적으로 판단하고 요청한 것입니다 — 기본값을 이렇게 바꾼 것이지, 조용히 끼워 넣은 변경이 아닙니다.

### 18-6-6. correct_answer 매칭 실패 재발 — 정규화 기반 완화 (2026-08-26)

18-6-3에서 correct_index를 correct_answer(정답 텍스트)로 바꾼 뒤에도, 검수 LLM 호출(18-6-5)을 붙이고 나서 Stanley가 영어 과목 "to부정사" 단원 퀴즈를 생성하다가 "3번 문항의 정답(correct_answer)이 보기 중에 없어요"라는 오류를 다시 겪었습니다 — 재시도 1회까지 소진하고도 실패한 사례입니다. 실제 원문 응답을 직접 확인하지는 못했지만, 영어 지문이라는 공통점에서 LLM이 정답 문구를 옮겨 적을 때 앞뒤에 따옴표를 붙이거나(`"interested in playing"`), 대소문자를 바꾸거나, 끝에 마침표를 붙이는 식으로 options 배열의 원문과 글자 단위로 완전히 동일하지 않게 재서술했을 가능성이 높다고 봤습니다(어디까지나 가설이며 확정 원인은 아닙니다 — 원문 로그가 없어 재현은 못 했습니다).

- `_normalize_for_match()`를 추가했습니다 — 양끝 공백 제거, 따옴표류(`'` `"` 및 유니코드 스마트 따옴표 `'` `'` `"` `"`)와 마침표/쉼표 제거, 소문자 변환을 순서대로 적용합니다.
- `_parse_quiz_json()`의 correct_answer 매칭을 2단계로 바꿨습니다 — ① 기존과 동일하게 옵션 텍스트와 완전 일치를 먼저 시도합니다. ② 실패하면 정답 문구와 모든 옵션을 각각 정규화한 뒤 비교해서, **정확히 하나의 옵션과만** 일치하는 경우에 한해 그 옵션을 정답으로 채택합니다. 정규화 후에도 일치하는 옵션이 없거나(전혀 다른 답) 둘 이상이면(모호함, 예: "Apple"과 "apple"이 둘 다 옵션에 있는데 정답이 "APPLE") 이전과 동일하게 `QuizError`를 올려 재시도를 유도합니다.

이 완화는 "겉모양만 다르고 실제로는 같은 답"인 경우만 구제하는 걸 목표로 했습니다 — 정답이 실제로 다르거나 어느 옵션을 가리키는지 애매한 경우까지 억지로 통과시키면 18-6-3에서 고친 "정답이 실제로 틀림" 문제가 다른 형태로 재발할 수 있기 때문입니다. 그래서 정규화 후 비교도 "정확히 하나만 일치"를 요구하도록 의도적으로 보수적으로 짰습니다.

### 18-6-7. 해설(explanation)에 깨진 LaTeX 표기가 섞이는 문제 (2026-08-26)

Stanley가 Google Forms에서 수학 퀴즈를 확인하다가 해설에 "전체 imesrac{30}{100}=12"처럼 알아볼 수 없는 텍스트가 섞여 나오는 걸 발견했습니다. 원래 LLM이 쓰려던 건 "전체 × 30/100 = 12"를 LaTeX 문법으로 "전체 \times \frac{30}{100} = 12"라고 표기한 것으로 보이는데, JSON으로 답할 때 백슬래시를 이스케이프하지 않고 그대로 냈습니다. 문제는 JSON 표준에서 `\t`와 `\f`가 그 자체로 유효한 이스케이프 문자(각각 탭, 폼피드)라는 점입니다 — `\times`는 `\t` + "imes"로 해석돼서 파싱 과정에서 탭 문자만 남고 "t"가 사라지고, `\frac`도 `\f` + "rac"로 같은 방식으로 망가집니다. JSON 자체는 완전히 유효한 형태라서(파싱 에러가 안 남) 기존 `try/except json.JSONDecodeError` 처리로는 이 손상을 전혀 잡아내지 못했습니다.

두 가지로 대응했습니다.

- **프롬프트(근본 원인 예방)**: `_build_prompt()`에 규칙 (5)를 추가해 LaTeX나 마크다운 수식 문법을 쓰지 말고 ×, ÷, / 같은 일반 기호와 "30/100" 같은 평문 표기만 쓰라고 명시했습니다. 다만 프롬프트 지시는 18-6-4에서 이미 확인했듯 100% 지켜진다는 보장이 없습니다.
- **코드(안전망)**: `_UNEXPECTED_CONTROL_CHAR_RE`를 추가해서, 파싱된 question/options/explanation 안에 줄바꿈(`\n`) 이외의 제어문자(탭, 폼피드 등)가 하나라도 남아있으면 이 손상 패턴으로 간주하고 `QuizError`를 올립니다. 제어문자가 남았다는 건 원래 백슬래시+글자 조합이 이미 사라진 뒤라는 뜻이라 어떤 LaTeX 명령이었는지 역추적해서 고쳐 쓸 방법이 없고, 그래서 복구를 시도하지 않고 기존 1회 재시도 정책에 태워서 다시 생성하게 했습니다.

18-6-3(correct_answer 매칭)과 마찬가지로 이 검증은 "계산이 맞는지" 같은 판단이 아니라 "있으면 안 되는 제어문자가 섞였는지"를 보는 순수하게 기계적인 검사라서, 검수 LLM 호출(18-6-5)보다 비용이 훨씬 싸고 오탐 위험도 낮습니다 — 그래서 검수 LLM에 맡기지 않고 별도의 기계적 검증으로 처리했습니다.

### 18-6-8. 문항이 없는 그래프/표를 참조하는 문제 (2026-08-26)

Stanley가 수학 퀴즈에서 "다음은 어느 학급에서 조사한 일주일 동안 읽은 책의 수를 나타낸 상대도입니다. 월요일에는 몇 권을 읽었습니까?"로 시작하는 문항을 보고 지적했습니다 — 이 프로젝트의 Quiz는 텍스트만 Google Forms에 반영하고, 그래프나 표를 이미지로 만들어 별도로 첨부하는 기능 자체가 없습니다(18-1 참고, Forms API로 텍스트/보기만 씁니다). 그런데 문항 문장은 "다음은 ~을 나타낸 상대도입니다"처럼 어딘가에 학생이 볼 그래프가 있다는 걸 전제하고 있어서, 실제로는 그 그래프가 어디에도 없어 학생이 풀 수 없는 문항이 만들어졌습니다.

두 가지로 대응했습니다.

- **프롬프트(근본 원인 예방)**: `_build_prompt()`에 규칙 (6)을 추가해 "이 퀴즈는 텍스트만 표시되고 이미지·그래프·표·도표를 별도로 첨부하지 않는다"는 사실을 명시하고, 표나 그래프로 보여줄 법한 데이터가 필요하면 그 데이터를 문항 문장 안에 직접 숫자/텍스트로 다 적으라고 지시했습니다(예: "어느 반의 요일별 독서량은 월 8권, 화 12권, ...입니다").
- **코드(안전망)**: `_VISUAL_REFERENCE_RE`를 추가해서 "다음/위/아래 그래프·표·도표·차트·그림·사진·지도를 보고/참고하여", "~을 나타낸 그래프/표/..." 같은 흔한 참조 표현이 question이나 explanation에 남아있으면 QuizError를 올려 재시도를 유도합니다. 특정 값이 맞는지 판단하는 게 아니라 정해진 표현 패턴만 찾는 순수 문자열 매칭이라(18-6-7의 제어문자 검증과 같은 성격), 검수 LLM 호출(18-6-5)보다 훨씬 저렴하고 오탐 위험도 낮습니다.

이 검증은 완벽하지 않습니다 — 정규식이 못 잡는 다른 표현("이 도표에서", "사진 속 인물은" 등)으로 시각 자료를 참조하면 그대로 통과할 수 있습니다. 근본적으로 해결하려면 실제로 차트 이미지를 생성해서 Forms에 첨부하는 기능이 필요한데, 이번 프로젝트 범위와 "크레딧 없이 개발" 방침(8-5)에는 맞지 않아 지금은 "애초에 시각 자료가 필요 없는 문항만 내게 하는" 방향으로 대응했습니다.
...
- **문항의 계산/사실관계 정확성은 100% 보장되지 않습니다** (18-6-4, 18-6-5 참고). 정답 인덱스가 해설과 어긋나는 문제(18-6-3)와 보기 동점 문제(18-6-4)는 코드로 기계적으로 걸러지고, "계산 자체를 틀리고 확신하는 경우"나 "사실관계·개념 분류상 정답이 하나로 안 좁혀지는 경우"(예: 완전변태 곤충 문항의 정답 후보가 둘이었던 사례, 형용사적/부사적 용법 분류가 틀렸던 사례)는 검수 LLM 호출(18-6-5)이 한 번 더 걸러냅니다. 다만 검수도 같은 LLM이 하는 판단이라 100% 보장은 아니고(검수 호출 자체가 실패하면 조용히 건너뛰기도 합니다, 18-6-5 참고), 재발 가능성이 완전히 없다고는 할 수 없으므로 생성된 문항은 교사가 Forms에서 한 번 훑어보고 쓰는 걸 권장합니다. 검수 단계 추가로 최악의 경우 LLM 호출이 최대 4회(생성 2회+검수 2회)까지 늘어날 수 있습니다(18-6-5).
